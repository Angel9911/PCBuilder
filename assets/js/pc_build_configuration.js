...
import {
    initMobileSummaryPanel,
    setupInitialQuestionListeners,
    updateBuildSummaryState,
    calculateTotalRangePrices,
    calculatePowerWattage,
} from 'app/configurator_summary';

import {
    triggerBottleneckAICheck,
    triggerFpsCalculation
} from 'app/bottleneck_configurator';
...
function init() {
    ...
    function resetAiQuestionnaire() {
        const btn = document.getElementById('start-config');
        if (!btn) return;                     // nothing to bind on this page/state
    ...
    // move this inside init and guard it:
    if (buildAiButton) {
